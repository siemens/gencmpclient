...
B<cmpClientDemo>
[B<imprint|bootstrap|update|revoke|pkcs10>]
[B<options>]
...
The B<cmpClientDemo> command is a demo client implementation for the Certificate
...
Define C<use_case> of the cmpClient application. The corresponding CMP request
will be executed with default settings. These settings could be adapt via the
file 'config/demo.cnf'.

=item B<options>

List of options implemented in the C<cmpClient> application.
...
CMP command to execute. Overwrites C<use_case> if present.
