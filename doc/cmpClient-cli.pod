...
The empty name (NULL-DN) can be explicitly given as a single slash: 'I</>'.
...
if present, the B<-issuer> option if present,
the issuer of the certificate given with the B<-oldcert> option if present,
the issuer of the CMP client certificate (B<-cert> option) if present,
...
unless the B<-secret> option indicating PBM or B<-unprotected_requests> is given.
