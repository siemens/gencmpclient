...
#define load_key_pwd(uri, format, pass, e, desc) \
    CREDS_load_key(app_get0_libctx(), app_get0_propq(), uri, format, false, pass, desc)
...
#define load_cert_pwd(uri, source, desc, type_CA, vpm) \
    CREDS_load_cert(app_get0_libctx(), app_get0_propq(), uri, \
...
#define load_crl(uri, format, stdin, timeout, desc, vpm)                \
    CREDS_load_crl(app_get0_libctx(), app_get0_propq(), uri, format, stdin, timeout, desc, vpm)
...
#define load_crls(files, format, timeout, desc, vpm) \
    CREDS_load_crls(app_get0_libctx(), app_get0_propq(), files, format, timeout, desc, 0, vpm)
...
CREDENTIALS *CREDS_load(OPTIONAL OSSL_LIB_CTX *libctx, const char *propq,
                        ...
                        OPTIONAL X509_VERIFY_PARAM *vpm);
#define load_creds(certs, key, source, desc, vpm) \
