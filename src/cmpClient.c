...
static SSL_CTX *setup_TLS(STACK_OF(X509) *untrusted_certs)
{
    ...
    if (opt_tls_trusted != NULL) {
        tls_trust = load_certstore(opt_tls_trusted, opt_otherpass, "trusted certs for TLS level",
    ...
    if (opt_tls_extra != NULL) {
        STACK_OF(X509) *tls_extra = load_certs_pwd(opt_tls_extra, opt_otherpass,
...
static X509_STORE *setup_CMP_truststore(const char *trusted_cert_files)
{
    ...
    X509_STORE *cmp_truststore =
        load_certstore(trusted_cert_files, opt_otherpass, "trusted certs for CMP level",
                       NULL /* no vpm: prevent strict checking */);
...
static int setup_ctx(CMP_CTX *ctx)
{
    ...
    if (opt_extracerts != NULL) {
        STACK_OF(X509) *certs =
            load_certs_pwd(opt_extracerts, opt_otherpass, "extra certificates for CMP",
                                  ...
                                  OPTIONAL LOG_cb_t log_fn)
{
    ...
    untrusted_certs = opt_untrusted == NULL ? NULL :
        load_certs_pwd(opt_untrusted, opt_otherpass, "untrusted certs", 1 /* CA */, vpm);
...
#define STR_OR_NONE(s) (s != NULL ? s : "(none)")
    ...
    if (opt_untrusted != NULL &&
        (untrusted = load_certs_pwd(opt_untrusted, opt_otherpass, "untrusted certs",
                                      ...
                                      enum use_case use_case)
{
    ...
    if (use_case == pkcs10 || use_case == revocation || use_case == genm) {
        ...
        if (opt_newkeytype != NULL || opt_centralkeygen) {
            ...
            *new_pkey = load_key_pwd(file, FORMAT_PEM, pass, NULL, desc);
            if (*new_pkey == NULL) {
                ERR_clear_error();
                desc = opt_csr == NULL
                    ? "fallback public key for cert to be enrolled"
                    : "public key for checking cert resulting from p10cr";
                pkey = FILES_load_pubkey_ex(app_get0_libctx(), app_get0_propq(), file,
                                            FORMAT_PEM, false, pass, desc);
    ...
    case NID_id_it_crlStatusList:
        ...
        }
        {
            ...
            if (opt_oldcrl == NULL) {
                ...
                oldcrl = FILES_load_crl_ex(app_get0_libctx(), app_get0_propq(),
                                           opt_oldcrl, FORMAT_ASN1, false, (int)opt_crls_timeout,
...
int main(int argc, char *argv[])
{
    ...
    if (!set_base_ui_method(NULL) || !setup_ui_method()) {
    ...
    if (opt_crls != NULL) {
        crls = FILES_load_crls_ex(app_get0_libctx(), app_get0_propq(),
                                  opt_crls, FORMAT_ASN1, (int)opt_crls_timeout, "pre-determined CRLs");
        if (crls == NULL)
            goto end;
    }
