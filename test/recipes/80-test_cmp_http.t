...
use lib "util/perl"; # needed in genCMPClient project
use strict;
use warnings;

use POSIX;
sub data_dir { return "../test/recipes/80-test_cmp_http_data" }
sub result_dir { return "." }
use OpenSSL::Test qw/:DEFAULT cmdstr data_file srctop_dir bldtop_dir/;
...
$server_port = 0; # dummy value for cmp_basic_tests
my @cmp_basic_tests = (
...
sub use_server_internally
{
    ok(run(cmd(["true"]))); # dummy since cmpClient does not support -use_server
}
...
indir data_dir() => sub {
    plan tests => 1 + @server_configurations * @all_aspects
        + 2
        - (grep(/^Mock$/, @server_configurations) && $mock_openssl_version < 3.0
           && grep(/^certstatus$/, @all_aspects));

    indir "Mock" => sub {
        test_cmp_http_aspect("basic", "options", \@cmp_basic_tests);
        use_server_internally();
    };
        ...
        load_config($server_name, $server_name);
        {
          SKIP: {
            ...
            foreach my $aspect (@all_aspects) {
                ...
                if ($server_name eq "Mock" && $aspect eq "certstatus" && $mock_openssl_version < 3.0) {
...
sub start_server {
    ...
    my $cmd = "$openssl cmp -config server.cnf $args";
